...
description: AGENTS.md "문서 매핑" 표에 등록된 작업 문서들을 **기존 포맷을 그대로 유지한 채** 최신 코드 상태와 사용자 입력에 맞춰 보강·갱신합니다. 매핑 표가 진실의 원천이며, 거기 적힌 경로의 문서만 대상으로 한다 (`.Codex/`든 `docs/`든 사용자가 지정한 어떤 디렉토리든). 새 문서를 만들지 않고, 매핑되지 않은 문서를 손대지 않으며, 자동 적용 없이 항상 사용자 승인을 받는다. 트리거 - "문서 동기화", "Codex 문서 업데이트", "source map 갱신", "/update-project-docs".
...
AGENTS.md "문서 매핑" 표에 등록된 문서들이 코드와 어긋날 때, **기존 포맷을 그대로 따라** 새 정보를 추가하거나 구식 항목을 갱신한다.

## 핵심 원칙

1. **매핑 표가 진실의 원천** — `AGENTS.md`의 "## 문서 매핑" (또는 "## Document Mapping") 표에 적힌 경로의 문서만 손댄다. 매핑되지 않은 문서는 건드리지 않고, 새 문서를 만들지도 않는다.
...
### Step 1 — AGENTS.md 매핑 표에서 대상 추출

```bash
# AGENTS.md 위치 결정 (루트 우선, 없으면 .Codex/)
CLAUDE_PATH=""
[ -f "$PWD/AGENTS.md" ] && CLAUDE_PATH="$PWD/AGENTS.md"
[ -z "$CLAUDE_PATH" ] && [ -f "$PWD/.Codex/AGENTS.md" ] && CLAUDE_PATH="$PWD/.Codex/AGENTS.md"
```

`AGENTS.md`의 "## 문서 매핑" (또는 "## Document Mapping") 섹션 안의 표에서 **첫 컬럼의 백틱 경로**를 모두 추출하여 `TARGETS` 리스트를 만든다.
...
다음 경우 즉시 종료한다:
- `AGENTS.md`가 없음 → "AGENTS.md를 먼저 설치하세요. `/project-setup` 또는 `/recommend-project-setting` 참고."
- "문서 매핑" 섹션이 없거나 표가 비어 있음 → "매핑 표가 비어 있습니다. AGENTS.md에 갱신 대상 문서를 등록한 뒤 다시 실행하세요."
- 추출된 경로 중 실제 파일이 하나도 없음 → 매핑 표에서 제거를 제안하고 종료

> AGENTS.md 자기 자신은 보통 매핑 표에 등록되지 않으므로 기본 대상에서 제외된다. 인수 `Codex`로 명시하거나 매핑 표에 직접 등록된 경우에만 대상에 포함한다.
...
2. 약식 — `source-map`, `db-schema`, `deploy`, `overview`, `design`, `Codex`
3. 대소문자 무시 부분 일치
...
### 대상
- `docs/SOURCE_MAP.md` (AGENTS.md 매핑 표에서 발견)
- `docs/DB_SCHEMA.md` (AGENTS.md 매핑 표에서 발견)
...
- 표 컬럼 수 일관성
- 닫히지 않은 코드 블록 / 인라인 코드
- 매핑 표가 자기 자신을 깨뜨리지 않음 (AGENTS.md를 손댄 경우)
...
AGENTS.md 매핑 표에 등록된 문서들이 최신 상태입니다.
```
...
3. **AGENTS.md 자체** — 기본적으로 대상에서 제외. 사용자가 인수 `Codex`로 명시하거나 매핑 표에 직접 등록한 경우에만 손댄다. 도메인 규칙·정보 소유권 변경은 자동 판단 불가하므로 전적으로 사용자 질의 모드로 동작한다.
...
| File | Purpose |
|------|---------|
| `<루트>/AGENTS.md` 또는 `<루트>/.Codex/AGENTS.md` | "문서 매핑" 표 — **이 스킬의 진실의 원천** |

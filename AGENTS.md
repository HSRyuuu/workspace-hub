...
- **두 호출자**: Tauri UI는 `core`를 라이브러리로 직접 호출. Codex 스킬은 같은 `core`를 감싼 `workspace-hub` CLI 바이너리를 호출 (JSON I/O 기본, `--human` 플래그).
...
이 문서는 Codex가 매 세션에 가장 먼저 읽는다. 따라서 **고정된 규약과 진입점만** 두고, 영역별 상세 지침은 아래 [문서 매핑](#문서-매핑) 표에서 필요할 때 끌어다 쓴다 (점진적 공개).
...
- 사용자의 수정사항이 있을 경우 **`.Codex/LESSONS.md`** 에 정해진 패턴으로 업데이트하세요.
...
| `GUIDE_BUILD_AND_RUN.md`      | 빌드·실행 사전조건(아이콘, cargo PATH) + CLI/Tauri 실행 절차        |
| `.Codex/LESSONS.md`          | 사용자 피드백 누적, 자기개선 순환                                   |
